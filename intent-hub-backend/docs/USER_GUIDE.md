...
3. 系统识别 `SKILL.md`，生成 JSON draft，写入租户 `imports/`。  
4. 查看索引：`GET /tenant/skill-drafts`。  
5. 应用草稿：`POST /tenant/skill-drafts/apply`。  

`sync_mode` 说明：

1. `draft`：只产出草稿，不自动改正式 routes。  
2. `apply`：扫描后自动导入正式 routes。  

## 8. CLI 使用

先安装后端包（确保有 `intent-hub` 命令），然后：
