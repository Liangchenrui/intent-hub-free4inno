...
# New tests for Task 1: Skill source model with logical-source fields
def test_skill_source_record_has_source_label_and_client_path_hint():
    """Test that SkillSourceRecord model supports source_label and client_path_hint fields."""
...
def test_skill_source_record_defaults_for_backward_compatibility():
    """Test that SkillSourceRecord defaults source_label from path basename."""
    # Test with path that has basename
    source1 = SkillSourceRecord(
        source_id="src_001",
        path="/some/path/to/skills",
        enabled=True,
        sync_mode="scan"
    )
    # With current model this will fail - source_label doesn't exist
    # After implementation, should default to "skills" (basename of path)
    
    # Test with empty source_label should use basename
    source2 = SkillSourceRecord(
        source_id="src_002",
    ...
    # Should default to "directory" (basename of path)


def test_create_skill_source_sets_source_label_from_basename(test_dir):
    """Test that create_skill_source defaults source_label from path basename."""
    platform_dir = test_dir / "platform"
    platform_dir.mkdir(parents=True, exist_ok=True)
    tenants_file = platform_dir / "tenants.json"
    ...
    # Create skill source with just path - should default source_label
    ...
    # After implementation, source.source_label should be "my_skills"
    # and client_path_hint should be None or empty


def test_create_skill_source_with_explicit_source_label(test_dir):
    """Test that create_skill_source accepts explicit source_label."""
    platform_dir = test_dir / "platform"
    platform_dir.mkdir(parents=True, exist_ok=True)
    tenants_file = platform_dir / "tenants.json"
    tenants_file.write_text("[]", encoding="utf-8")
    ...
    # Create skill source with explicit source_label
    # This test will need to be updated when create_skill_source
    # accepts source_label parameter
    pass


def test_backward_compatibility_with_existing_json(test_dir):
    """Test that existing JSON data without new fields still loads."""
    platform_dir = test_dir / "platform"
    platform_dir.mkdir(parents=True, exist_ok=True)
    
    # JSON without source_label or client_path_hint (old format)
    tenants_file = platform_dir / "tenants.json"
    tenants_file.write_text(
        json.dumps(
            [
    ...
    # After implementation, source_label should default from path basename
